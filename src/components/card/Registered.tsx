...
export function Registered({
		...
		for (let index in schemaDays.options) {
			...
			for (let key of Object.keys(option.registered)) {
				if (option.registered[Number(key)].note === `gsr-${localStorage.getItem('userEmail')}`) {
					state = true;
					setRegistered((prevRegistered) => [...prevRegistered, [Number(index), Number(key)]]);
				}
			}
		}
		setIsRegister(state);
	...
	const unregister = async (unregisterIndex: any, reset = false) => {
		try {
			...
			let schemaDaysUpdate = await sheetApi.unregister(
				sheetIndex,
				cardIndex,
				columnStart,
				schemas,
			...
			schemasCopy[Object.keys(schemas)[sheetIndex - sheetApi.disabled]].schema[
				Object.keys(schemaDay)[cardIndex]
			] = schemaDaysUpdate;
		...
		<div className="card_unregister">
			<ul className="card_unregister_item">
				{registered.map((index: any, arrayIndex) => (
					<li key={arrayIndex}>
						{schemaDays.options[index[0]].registered[index[1]].value}{' '}
						{isLocked ? null : (
							<a className="card_unregister_action" onClick={() => unregister([index])}>
								<MdCancel />
							</a>
						)}
					</li>
			...
			{isLocked ? null : (
				<a className="card_unregister_global" onClick={() => unregister(registered, true)}>
					Cancel
				</a>
			)}
		</div>
